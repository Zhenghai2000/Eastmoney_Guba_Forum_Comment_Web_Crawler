...
#加载未填充的codes
codes = os.listdir("preliminary_data")
ok = os.listdir("temporary_data")
ok = [i.split("_")[1] for i in ok]
codes = [i for i in codes if i not in ok]

#代理接口
pr = Proxy_request()
#填充年份
dates = pd.date_range(start="1800-01-01",end = "2022-12-30",freq="1D").to_list()
dates.reverse()
...
for code in tqdm(codes):
    try:
        #读取items
        items = read_items(csv="preliminary_data/"+code)
        #剔除有毒数据
